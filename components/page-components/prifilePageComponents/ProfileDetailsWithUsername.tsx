...
const ProfileDetailsWithUsername = ({username}:{username: string}) => {
  ...
  `
  const { data }: { data: any } = useSuspenseQuery(query, { variables: { username } })
  console.log(data)

  const [opt, setOpt] = useState(1)

  const newDate = new Date(Number(data.fetchUserDetailsWithUsername.createdAt))
  console.log(newDate)
  ...
  return (
    <main className='w-full'>
      <div className='flex gap-3 items-center w-full p-2 sticky top-0 bg-black/70 backdrop-blur-md z-10'>
        ...
        <div className=''>
          <p className='font-semibold'>{data.fetchUserDetailsWithUsername.name}</p>
          <p className='text-sm opacity-70'>{data.fetchUserDetailsWithUsername.tweetCount} Tweets</p>
        </div>
      </div>
      <Image 
      src={data.fetchUserDetailsWithUsername.banner}
      ...
      <div className='w-full h-24 p-3 px-7 flex justify-between bg-slate-300/5'>
        <Image 
        src={data.fetchUserDetailsWithUsername.profileImageUrl}
      ...
      <div className='min-h-28 px-7 bg-slate-300/5'>
        <p className='text-xl font-semibold'>{data.fetchUserDetailsWithUsername.name}</p>
        ...
        <p className='text-sm opacity-50 mt-1 flex'>
          ...
          {
            new Date(Number(data.fetchUserDetailsWithUsername.createdAt)).toLocaleString('en-GB', {
              hour12: false,
            }).split(",")[0]
          }
        </p>
        <div className='flex gap-7 mt-1 pb-1'>
          <div className='flex hover:underline opacity-70 text-white/50'>
            <p className='font-semibold opacity-100 text-white'>{data.fetchUserDetailsWithUsername.followingCount}</p> &nbsp; following
          </div>
          <div className='flex hover:underline opacity-70 text-white/50'>
            <p className='font-semibold opacity-100 text-white'>{data.fetchUserDetailsWithUsername.followersCount}</p> &nbsp; following
          </div>
        </div>
      </div>
      <section>
...
export default ProfileDetailsWithUsername